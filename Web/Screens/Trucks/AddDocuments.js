...
const COLORS = {
  ...
  // In your AddDocumentForm component, make sure you have both functions:
const handleSubmit = async () => {
  ...
  try {
    ...
    // Handle file upload with blob approach (similar to your driver documents)
    if (selectedFile) {
    ...
    if (response.ok) {
      ...
      setTimeout(() => {
        Alert.alert('Succes', 'Documentul a fost adăugat cu succes!');
      }, 100);
    ...
    Alert.alert('Eroare', `Failed to add document: ${error.message}`);
...
// Also, update your component props to accept onDocumentAdded:
// const AddDocumentForm = ({ isVisible, onClose, onDocumentAdded, truckId, authTokenForm }) => {
  const handleCancel = () => {
    onClose();
  };
